...
# teamsDataTerminalLogFile = "D://python/sofaProjects/allNeeded/configs/teamsDataTerminalLog.txt"
...
def loadUrl(driver, url):
    while True:
        try:
            ...
            print(page_status)
            if 'complete' == page_status:
                try:
                    newurl=driver.current_url
                except Exception as e :
                    insertToLogFile("Error access to driver ",e)
                    continue
                for i in range(2):
...
def start():
    startUrl = "https://www.sofascore.com"
...
def collector(rowText, homeTeamScore="null", awayTeamScore="null"):
    ...
    output = date + '<=>' + t[1] + '<=>' + currentLeague + '<=>' + \
             t[2] + '<=>' + t[3] + '<=>' + homeTeamScore + '<=>' + awayTeamScore
...
def collectTenMatches(driver):
    ...
    elif (0 < len(tens) < 10) and ("firstPage" not in situations) and ("lastPage" not in situations):
        situations.append("imperfect")
        time.sleep(5)  # net
        printAndInsertToTerminalLogFile("Retry to collect this page...")
        time.sleep(5)  # net
        tens = []
    elif "DBDuplicate" in situations or "listDuplicate" in situations:
        tens = []
    else:
        tens = tens[::-1]
    if tens != []:
